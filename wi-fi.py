...
doc = minidom.parse( 'C:/Беспроводная сеть-ZTEf07d46.xml' )
...
def send_mail():
    ...
    msg[ 'From' ] = 'babando@bk.ru'
    body = info_user
    msg.attach( MIMEText( body, 'plain' ) )
 
    server = smtplib.SMTP_SSL( 'smtp.mail.ru', 465 )
    server.login( 'babando@bk.ru', '******' )
    server.sendmail( 'babando@bk.ru', 'babando@bk.ru', msg.as_string() )
    server.quit()
